...
fn bench_nexus_parser_detect_format(c: &mut Criterion) {
    let test_data = prepare_test_data();

    let mut group = c.benchmark_group("nexus_parser_detect_format");
    ...
    for (name, nexus_string) in &test_data {
        ...
        let _ = group.bench_with_input(
            BenchmarkId::new("parse_nexus_detect_format", name),
            nexus_string,
            |b, nexus| {
                b.iter(|| {
                    let _ = black_box(parse_trees(nexus.clone()));
                });
            },
        );
    }
...
criterion_group!(
    ...
    targets = bench_nexus_parser_detect_format
);
