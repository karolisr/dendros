//! Attribute system for phylogenetic tree annotations
//!
//! This module provides a flexible attribute system for storing metadata
//! associated with phylogenetic tree nodes and branches. It supports
//! multiple data types including integers, decimals, colors, text, and lists.
//!
//! The system includes automatic type unification (e.g., Integer + Decimal → Decimal)
//! and comprehensive parsing from various string formats including NEWICK annotations.
...
// Section 3: AttributeValue implementations
...
// Section 4: Attribute implementations

impl Attribute {
    /// Get the type signature of this attribute
...
impl From<String> for Attribute {
    fn from(s: String) -> Self {
        // Use FromStr implementation which handles lists, colors, and other formats
        match s.parse() {
            Ok(attr) => attr,
...
/// Parse range syntax: {item1,item2,...}
fn parse_range_syntax(inner: &str) -> Result<Vec<AttributeValue>, ()> {
    if inner.is_empty() {
        return Ok(Vec::new());
    }
...
impl FromStr for Attribute {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // Handle curly brace syntax {item1,item2,...} - convert to list
        else if s.starts_with('{') && s.ends_with('}') {
            let inner = &s[1..s.len() - 1];
            let values = parse_range_syntax(inner)?;
            Ok(Attribute::List(values))
        } else {
            // Handle simple values: check for hex color, then try integer, then decimal, then text
            if let Ok(integer) = s.parse::<TreeInt>() {
